...
系统维护一组人工核对的核心模型记录，并从论文索引中为明确提出模型或模型级方法的论文生成轻量模型记录；当前数据覆盖 2022-2026，包含 90+ 模型记录和 120+ 篇 VLA 模型/方法论文。论文索引侧重 VLA 模型、机器人基础模型、语言条件策略、action tokenization，以及模型适配/加速方法。纯数据集、benchmark 和平台介绍不作为主记录。
