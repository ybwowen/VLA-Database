...
系统维护一组人工核对的核心模型记录，并从论文索引中的 `model_names` 字段为明确提出模型或模型级方法的论文生成轻量模型记录；当前数据覆盖 2022-2026，包含 150+ 模型记录和 145+ 篇 VLA 模型/方法论文。论文索引侧重 VLA 模型、机器人基础模型、语言条件策略、action tokenization，以及模型适配/加速方法。纯数据集、benchmark 和平台介绍不作为主记录。
...
- 作者信息优先来自 arXiv、PMLR、RSS 或项目页；机构信息只在来源给出可靠映射时写入。
...
- Poster 可直接使用项目运行截图作为展示材料。

## 15. 不足与后续扩展

- 仍有部分论文的 affiliation 来源没有公开逐作者映射，因此会保留为空。
- 管理端还没有登录权限控制。
...
本项目实现了一个完整可运行的 VLA 模型数据库原型，覆盖关系数据库设计与实现的核心要求。系统具备：
...
因此，它既可以作为关系数据库系统案例，也可以作为后续整理 VLA 资料库的基础。
